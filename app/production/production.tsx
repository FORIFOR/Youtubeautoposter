...
export default function Production(){
  ...
  return <main className="production"><header><a href="/workflow">← 制作・改善フロー</a><span>Growth Studio / PRODUCTION</span></header>
    ...
    {!state?<p>読み込み中… 保存には<a href="/signin-with-chatgpt?return_to=/production">サインイン</a>が必要です。</p>:<>
      <section className="production-status"><strong>{online?"制作ワーカーから応答あり":"制作ワーカーの応答なし"}</strong><p>台本 {state.worker?.text?"設定あり":"未設定"} / 画像・音声 {state.worker?.media?"設定あり":"未設定"} / 動画保存 {state.assetsConfigured?"設定あり":"未設定"} / アップロード {state.uploadEnabled?"許可あり":"停止中"}</p><small>設定は実API接続の成功を意味しません。新規制作は1日{state.dailyJobLimit}件まで（UTC）。実際の請求上限はプロバイダー側で設定してください。</small></section>
      <section><h2>01　根拠から台本を書く</h2><label>元の下書き<select value={draft?.id??""} onChange={e=>setSelected(e.target.value)}><option value="" disabled>下書きを選択</option>{state.drafts.map(d=><option value={d.id} key={d.id}>{d.title}</option>)}</select></label>
        <label className="check"><input type="checkbox" checked={consent} onChange={e=>setConsent(e.target.checked)}/>選んだ原稿・根拠をOpenAIへ送り、台本生成のAPI費用を許可する</label>
        <button disabled={busy||!draft||!consent||!state.enabled} onClick={()=>draft&&void act({action:"generate",draftId:draft.id,expectedRevision:draft.revision,consentExternal:true,consentCost:true})}>台本と場面設計を生成</button>
        <small>同じ版からの重複要求は再生成しません。再生成する場合は元の下書きを編集してください。</small></section>
...
function JobCard({job,busy,uploadEnabled,act}:{job:Job;busy:boolean;uploadEnabled:boolean;act:(body:unknown)=>Promise<void>}){
  const [media,setMedia]=useState(false),[review,setReview]=useState(false),[rights,setRights]=useState(false),[upload,setUpload]=useState(false),[editing,setEditing]=useState(false),[planText,setPlanText]=useState(""),[localError,setLocalError]=useState("");
  ...
  useEffect(()=>{if(job.plan)setMetadata(m=>({...m,title:job.plan!.title,description:job.plan!.description+"\n\n"+DISCLOSURE}));},[job.planHash]);
  const send=(action:string,extra:Record<string,unknown>={})=>act({action,id:job.id,version:job.version,...extra});
  return <section className="production-job"><div className="production-job-heading"><h2>{job.plan?.title??job.source.title}</h2><strong>{labels[job.stage]}</strong></div><small>投稿先：{job.source.channelId} · 元の台本 第{job.source.draftRevision}版</small>
    ...
    {job.stage==="planned"&&<><button disabled={busy} onClick={()=>{setEditing(!editing);setPlanText(JSON.stringify(job.plan,null,2));}}>台本・字幕・画像指示を編集</button>{editing&&<><textarea aria-label="構造化台本" rows={14} value={planText} onChange={e=>setPlanText(e.target.value)}/><button disabled={busy} onClick={()=>{try{const plan=JSON.parse(planText);setLocalError("");void send("edit-plan",{plan});}catch{setLocalError("JSONの形式を確認してください");}}}>編集を保存</button></>}{localError&&<p role="alert">{localError}</p>}
    ...
    {job.artifact&&<><h3>03　完成した映像と音声を確認</h3><video controls preload="metadata" src={`/api/production/${job.id}/video`} aria-label="完成MP4の確認"/><small>SHA-256：{job.artifact.sha256}<br/>{job.artifact.width}×{job.artifact.height} · {job.artifact.duration.toFixed(1)}秒 · {job.artifact.profile}</small></>}
    {job.stage==="ready"&&<div className="metadata"><label>タイトル<input value={metadata.title} maxLength={100} onChange={e=>setMetadata({...metadata,title:e.target.value})}/></label><label>説明<textarea rows={5} value={metadata.description} onChange={e=>setMetadata({...metadata,description:e.target.value})}/></label>
      ...
      <button disabled={busy||!review||!rights} onClick={()=>void send("approve",{videoHash:job.artifact?.sha256,metadata,reviewedVideoAndAudio:true,rightsConfirmed:true})}>この動画・公開内容を承認</button></div>}
    {job.approval&&<div><h3>04　承認した内容でアップロード</h3><p>{job.approval.metadata.title} / {job.approval.metadata.privacy} / 投稿先 {job.approval.channelId}</p><small>承認期限：{new Date(job.approval.expiresAt).toLocaleString("ja-JP")}</small>
      {(job.stage==="approved"||job.stage==="uncertain")&&<><label className="check"><input type="checkbox" checked={upload} onChange={e=>setUpload(e.target.checked)}/>上記の投稿先・公開範囲でYouTubeへ送信する</label><button disabled={busy||!upload||!uploadEnabled||Date.parse(job.approval.expiresAt)<=Date.now()} onClick={()=>void send(job.stage==="uncertain"?"resume-upload":"upload",{approvalHash:job.approval?.digest,confirmed:true})}>{job.stage==="uncertain"?"保存済みセッションだけ再開":"承認内容でアップロード"}</button></>}
