...
完了条件は、実動画を3本公開し、公開24時間・72時間後の反響を分析して、変更点を一つ反映した4本目を制作することです。3本の書き出しは完了しています。投稿先チャンネルは未設定で、YouTubeへの投稿、実測、改善版の制作はまだ行っていません。

新規チャンネルの作成はユーザーが許可済みです。「こぐまのポコのおはなし」／`@poco_ohanashi_jp` を作成フォームに入力しました。現在は最終ボタンに伴う利用規約同意の確認待ちで、作成はまだ確定していません。[設定案](channel.json)、[プロフィール画像](assets/poco-channel-avatar.png)、[画像生成プロンプト](prompts/channel-avatar.json)を保存しています。画像は組み込みの image_gen で制作し、YouTubeへの適用は未完了です。
...
1080×1920、30fps、H.264/AAC。日本語ナレーションと字幕、問いかけの待ち時間を含みます。絵本の場面にゆっくりズームする形式です。
...
現在の管理画面のA/B比較は各案4本を必要とします。今回の3本は探索として別に分析し、その判定条件を満たしたように表示しません。常時収集や後日の実行は、実際の公開時刻が確定してから接続・設定する必要があります。
